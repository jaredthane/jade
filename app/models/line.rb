...
#				#logger.debug  "self.last_change_type =" + self.last_change_type.to_s
	...
	def revenue_account(order=self.order)
		logger.debug "REady to try accounts"
  ...
  ###################################################################################
	...
	def product_name=(name)
		#logger.debug "Setting product name..................."
		#logger.debug "name=" + name.to_s
...
#			#logger.info "Product.find_by_upc(upc).id=#{Product.find_by_upc(upc).id.to_s}"
		...
		if self.order
			#logger.debug "we have an order"
			if self.order.order_type_id == 1
				#logger.debug "self.order.order_type_id" + self.order.order_type_id.to_s
