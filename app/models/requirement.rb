...
class Requirement < ActiveRecord::Base
	...
	def price(price_group = User.current_user.current_price_group)
		if required
			return ((required.price(price_group)||0) * -1 * (self.relative_price||0) - (self.static_price||0)) * (self.quantity||0)
		else
			return (-1 * (self.relative_price||0) - (self.static_price||0)) * (self.quantity||0)
		end
	end
end
