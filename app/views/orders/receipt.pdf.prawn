...
#			pdf.bounding_box([pdf.bounds.width/3+10, pdf.bounds.top-30], :width=>pdf.bounds.width/3, :height=>65) do
				#pdf.stroke_bounds
				pdf.font_size = 10
				pdf.text User.current_user.today.to_date, :at=>[pdf.bounds.left + 225,pdf.bounds.bottom+350]
				pdf.font_size = 12
				pdf.text client.name, :style=>:bold, :at=>[pdf.bounds.left + 225,pdf.bounds.bottom+330]
				pdf.font_size = 8
				pdf.text client.address, :at=>[pdf.bounds.left + 225,pdf.bounds.bottom+310]
				if client.id != 3
					pdf.font_size = 10
