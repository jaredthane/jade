...
for m in @movements    
  ...
  when Movement::SALE
  	data.unshift([m.id, "Venta", m.created_at.to_date, m.order.receipt_number, m.order.client.name, m.product.name, m.quantity, number_to_currency(m.quantity * m.cost), total[m.product], number_to_currency(value[m.product])])
  when Movement::PURCHASE
  	data.unshift([m.id, "Compra", m.created_at.to_date, m.order.receipt_number, m.order.vendor.name, m.product.name, m.quantity, number_to_currency(m.quantity * m.cost), total[m.product], number_to_currency(value[m.product])])
  when Movement::COUNT
...
if data.length>0
	pdf.table(data,
		:font_size => 8,
		:align => { 0 => :center, 1 => :left, 2 => :center, 3 => :center, 4 => :center, 5 => :center , 6 => :center , 7 => :center, 8 => :center },
		:headers => ['ID', 'Tipo', 'Fecha', 'Factura', 'Entidad', 'Producto', 'Cantidad', 'Valor', 'Inventario','Saldo'],
		:border_style => :grid,
		:column_widths => { 0 => 0.10*width, 1 => 0.10*width, 2 => 0.10*width, 3 => 0.10*width, 4 => 0.10*width, 5 => 0.10*width, 6 => 0.10*width, 7 => 0.10*width, 8 => 0.10*width, 9 => 0.10*width})
else
	pdf.font_size = 10
