...
def generic_show(request, object_id, Model):
	...
	current_url=request.get_full_path()
	current_lang=request.session['django_language']
	return render_to_response(Model._meta.app_label+'/show_'+Model._meta.object_name.lower()+'.html', {'obj': obj, 'current_url':current_url, 'current_lang':current_lang})
...
def generic_edit(request, Model, Form, object_id = None):
	...
	if request.method == 'POST': #POST
		...
		if object_id: #UPDATE
			...
			obj = get_object_or_404(Model, pk=object_id)
			print "obj=" + str(obj)
			current_url=reverse('show_'+Model._meta.object_name.lower(), kwargs={'object_id':obj.pk} )
...
#			print "obj.revenue_account=" + str(obj.revenue_account)
			...
			current_url=request.get_full_path()
			return render_to_response(Model._meta.app_label+'/edit_'+Model._meta.object_name.lower()+'.html', {'obj': (obj or None), 'form':form, 'current_url':current_url, 'current_lang':current_lang})
	else: # GET
		print "GETTING"
		current_url=request.get_full_path()
		if object_id: #EDIT
	...
	return render_to_response(Model._meta.app_label+'/edit_'+Model._meta.object_name.lower()+'.html', {'obj': obj, 'form':form, 'current_url':current_url, 'current_lang':current_lang})
